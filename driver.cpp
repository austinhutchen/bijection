...
int main(int argc, char *argv[]) {
  ...
  int curtime = time(NULL);
  if(argc<2){
    std::cout << "INVALID execution. Include filename using ./a.out {filename}"
<< std::endl; return 0;
      }
...
write, CLEAR to delete a file,and READ for read. Type LEAVE to exit program."<<
std::endl; std::cin >> choice; if (!std::cin.fail()) { if
(output.lowercase(choice) == "write") { std::cout << "Begin typing below. Type
ENTER to type on a new line, or EXIT to quit."<< std::endl; std::cin.ignore();
      while (getline(std::cin, input)) {
        if (input != "EXIT") {
          f = output.print(input);
        } else {
      ...
      if (choice == "y" || choice == "Y") {
        std::cout << "Enter the number of the line that you wish to edit." <<
std::endl; std::cin >> line; if (!std::cin.fail()) { output.edit(line);
          // here is where the editing happens, will return a value based on
          // edit success
        } else {
          std::cout << "ERROR" << std::endl;
    ...
    std::cout << "Please enter the phrase write or read for your choice. No
spaces allowed."<< std::endl; return main(argc,argv);
  }
  output.closefile();
}
*/